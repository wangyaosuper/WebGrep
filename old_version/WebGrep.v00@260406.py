...
def is_news_link(url):
    ...
    # 排除非新闻链接的路径
    excluded_paths_news = ['/tougao/', '/?page=', '/register', '/login', '/search', '/icp.html', '/homepage', '/dd_number/', '/special/', '/special_category/', '/creator/']
...
def main():
    ...
    # 保存到文件
    output_file = f"news_output_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
